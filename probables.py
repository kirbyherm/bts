...
def probables():
    ...
    for i in range(len(games)):
        ...
        df_temp = pd.DataFrame({"Team":pd.Series([away_team,home_team]), "Home":pd.Series([False,True]), "Opponent":pd.Series([home_team,away_team]), "Field":pd.Series([field,field]), "GameTime":pd.Series([gametime,gametime]), "PitcherLeftHand":pd.Series([home_pitcher_hand,away_pitcher_hand]), "PitcherERA":pd.Series([home_pitcher_era,away_pitcher_era]), "Playing":pd.Series([True,True])})
